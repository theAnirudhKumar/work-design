...
This skill owns the handoff procedure only. It does not restate rules that live elsewhere. Root `CLAUDE.md` owns routing and preferences, the workspace's own file-naming rules own placement, and its voice or style guide owns tone. Read those where the steps below point to them.

---

## Mode A: Handoff

Triggered when the user signals the chat is ending. Do not wait to be asked twice, and do not offer a conversational summary instead. Write the file.

### Step A1: Identify the project area

Read the routing rules in root `CLAUDE.md` and pick the project area this chat belongs to. If the chat spanned more than one, pick the area where most of the work landed and name the others in the Context Loaded section. If no area fits, use a general `Projects` folder.

State the chosen project area to the user before writing.
...
### Step A3: Write the handoff file

Name it `YYYY-MM-DD-descriptive-topic.md` using today's date, hyphens, no spaces. The topic must describe the work, not the session. Use `2026-07-30-hubspot-dashboard-arr-rebuild.md`, never `2026-07-30-chat-handoff.md`.

If a handoff for the same topic already exists from an earlier session, update that file rather than creating a second one, and record the refresh inside it. Two handoffs on one topic means the next chat reads the wrong one.
...
End by printing a short paste-ready line for the new chat, nothing more:

> Resume the [topic] handoff in [Project Area].

### Step A7: Confirm

List the file created or updated with its full path, and the index row added.
...
Read the master index at `<Ops Area>/Chat Context/chat-context-index.md`.
...
Read the project area's `CLAUDE.md` and `MEMORY.md` named in the handoff, plus every file listed under Context Loaded.
...
If something in the session is durable rather than session-scoped, a decision that outlives this thread of work or a new working convention, propose it as a `MEMORY.md` entry separately and wait for approval. Handoff and memory are different jobs. Do both when both apply.
