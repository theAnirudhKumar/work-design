...
Audits the tools you already pay for. Builds an inventory with cost, last use and overlap, gives every row a verdict of keep, cancel, downgrade or consolidate, and leads with the total it would free up. Ships an inventory table.
...
> Download the `stack-audit` skill from https://github.com/theAnirudhKumar/work-design/tree/main/skills/stack-audit, zip the `stack-audit` folder on its own, then upload it as a skill in Claude.
